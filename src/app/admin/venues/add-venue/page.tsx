...
import { z } from 'zod';
import { ArrowLeft, ArrowRight, Image as ImageIcon,  Save, 
  MapPin,  Building2, Users, Clock,Info,Star,Calendar,Pencil,DollarSign,CheckSquare,Trash2
} from 'lucide-react';
...
export default function AddVenuePage() {
  ...
  const onSubmit = async (data: VenueFormValues, isDraft = false) => {
    if (!venuesEnabled) {
      toast.showInfo('Temporarily Unavailable', 'Venue creation is currently disabled.');
      return;
    }

    setIsSubmitting(true);
    
    const finalStatus = isDraft ? "inactive" : data.status;
    
    try {
      const supabase = createClient();
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        toast.showError('Authentication Required', 'You must be logged in to create a venue');
        return;
      }
      ...
      if (featuredImageRef.current?.files?.[0]) {
        const file = featuredImageRef.current.files[0];
        featuredImageUrl = await uploadFileToSupabase(file, 'venues', 'featured', `venue-${Date.now()}`);
      }
      ...
      if (galleryImagesRef.current?.files) {
        ...
        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          const url = await uploadFileToSupabase(file, 'venues', 'gallery', `gallery-${Date.now()}-${i}`);
      ...
      const venueData = {
        ...
        map_coordinates: data.mapCoordinates || null,
        
        // Capacity
        capacity: data.capacity,
        seated_capacity: data.seatedCapacity || null,
        standing_capacity: data.standingCapacity || null,
        ...
        minimum_hours: data.pricing.minimumHours || null,
        
        // Policies
        cancellation_policy: data.policies.cancellationPolicy,
        min_booking_notice: data.policies.minBookingNotice,
        max_booking_advance: data.policies.maxBookingInAdvance,
        ...
        status: finalStatus,
        rules: data.rules || [],
        owner_notes: data.ownerNotes || null,
      ...
      console.log('Saving venue to Supabase:', venueData);
      ...
      toast.showSuccess(
        isDraft ? 'Draft Saved' : 'Venue Published', 
        isDraft ? 'Venue saved as draft' : 'Venue published successfully'
      );
      ...
      toast.showError('Creation Failed', `Failed to create venue: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const saveAsDraft = async () => {
    if (!venuesEnabled) {
      toast.showInfo('Temporarily Unavailable', 'Venue drafts are currently disabled.');
      return;
    }
    ...
    if (!currentData.name) {
      toast.showError('Name Required', 'Please provide at least a venue name for your draft');
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (featuredImageRef.current?.files?.[0]) {
        const file = featuredImageRef.current.files[0];
        featuredImageUrl = await uploadFileToSupabase(file, 'venues', 'featured', `draft-${Date.now()}`);
      }
      ...
      const venueData = {
        ...
        venue_type: currentData.venueType || null,
        
        // Location (can be null for drafts)
        address: currentData.address || null,
        city: currentData.city || null,
        country: currentData.country || null,
        postal_code: currentData.postalCode || null,
        map_coordinates: null,
        
        // Capacity
        capacity: currentData.capacity || 50,
        seated_capacity: currentData.seatedCapacity || null,
        standing_capacity: currentData.standingCapacity || null,
        ...
        // Pricing
        base_price: currentData.pricing?.basePrice || 0,
        currency: currentData.pricing?.currency || "GHS",
        pricing_model: currentData.pricing?.pricingModel || "hourly",
        minimum_hours: currentData.pricing?.minimumHours || null,
        
        // Policies
        cancellation_policy: currentData.policies?.cancellationPolicy || "flexible",
        min_booking_notice: currentData.policies?.minBookingNotice || 24,
        max_booking_advance: currentData.policies?.maxBookingInAdvance || 180,
        ...
        // Settings
        status: 'inactive',  // Drafts are inactive
        rules: currentData.rules || [],
        owner_notes: currentData.ownerNotes || null,
      ...
      console.log('Saving draft to Supabase:', venueData);
      
      // Insert draft venue into Supabase
      const { data: insertedVenue, error } = await supabase
        .from('venues')
        .insert([venueData])
        .select()
  ...
  const handleFeaturedImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!isValidSize) {
        ...
        toast.showError('Image Too Large', error || 'File size exceeds maximum limit');
        // Reset the input
        if (featuredImageRef.current) {
    ...
    return (
      <div className="space-y-6">
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          Adding venues is temporarily disabled while venue functionality is frozen.
        </div>
        <Link
          href="/admin"
          ...
          {formSteps.map((step, index) => (
            <div
              ...
              className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm whitespace-nowrap cursor-pointer flex-shrink-0 
                ${currentStep === index ? "bg-black dark:bg-black text-white" : "bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400"}`}
      ...
      {!previewMode ? (
          ...
          <form className="bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-800 p-4 sm:p-6">
            ...
            {currentStep === 0 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Venue Details</h2>
            ...
            {currentStep === 1 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Location</h2>
            ...
            {currentStep === 2 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Capacity & Features</h2>
            ...
            {currentStep === 3 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Pricing & Policies</h2>
            ...
            {currentStep === 4 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Venue Media</h2>
            ...
            {currentStep === 5 && (
              <motion.div
                ...
                className="space-y-6"
              >
                <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Venue Settings</h2>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-800 p-4 sm:p-6">
                      <div className="mb-4 pb-4 border-b border-neutral-200 dark:border-neutral-800">
                            <h2 className="text-xl font-semibold text-neutral-900 dark:text-gray-100">Venue Preview</h2>
