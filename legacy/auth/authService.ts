...
export async function verifyEmail(
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
        // For development, use hardcoded verification code
        const DEV_VERIFICATION_CODE = '123456';
        
        if (verificationCode !== DEV_VERIFICATION_CODE) {
    ...
    try {
      if (updatedRequest) {
        ...
        createLogEntry({
          ...
          details: `${adminName} ${status === 'approved' ? 'approved' : status === 'rejected' ? 'rejected' : 'revoked'} ${roleType} role request for ${finalUpdated.userName}${(status === 'rejected' || status === 'cancelled') && rejectionNote ? ' (Reason: ' + rejectionNote + ')' : ''}`,
